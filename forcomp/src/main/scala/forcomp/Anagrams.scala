...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    if (w == null || w.size == 0)  List()
    else {
      val wo = w.trim.toLowerCase groupBy(c => c) map(p => (p._1, p._2.length))
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val res = dictionary map (w => (wordOccurrences(w), w)) groupBy(p => p._1)
  ...
  def wordAnagrams(word: Word): List[Word] = {
    ...
    val res = dictionaryByOccurrences(wo)
    println(res)
    res
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    sentence match {
      case List() => List(List())
      case x::Nil => wordAnagrams(x)::Nil
      case x::xs => { println(x); wordAnagrams(x)::sentenceAnagrams(xs) }
    }
  }
}
